...
test('shortcut reminder follows actual global bindings and hides unknown or unbound actions', () => {
  const bind = {dispatcher:'__lua',arg:'133',description:'Toggle scratchpad',key:'S',modmask:64,submap:''};
  assert.equal(model.toggleShortcut([bind],'scratchpad'),'Super + S');
  assert.equal(model.toggleShortcut([{...bind,key:'F9',modmask:69}],'scratchpad'),'Super + Ctrl + Shift + F9');
  assert.equal(model.toggleShortcut([bind],'music'),'');
  assert.equal(model.toggleShortcut([{...bind,submap:'resize'}],'scratchpad'),'');
  assert.equal(model.toggleShortcut([{...bind,description:'Something else'}],'scratchpad'),'');
  assert.equal(model.toggleShortcut([],'scratchpad'),'');
  assert.equal(model.toggleShortcut([{...bind,dispatcher:'togglespecialworkspace',arg:'music',modmask:12}],'music'),'Ctrl + Alt + S');
});

...
ShellRoot {
  ...
  property string expectedLanguage: Quickshell.env("SCRATCHPEEK_EXPECTED_LANGUAGE") || "pl"
  readonly property bool startupCase: scenario.indexOf("startup-") === 0
  readonly property var startupEntry: ({id:"sarr.scratchpeek", workspace:"music", language:"pl",
    autoUpdates:false, compact:true, hintsMode:"auto", hintsUsed:37, uiScale:1.25,
  ...
  function finish() { console.info("SCRATCHPEEK_PREFERENCES_PASS"); Qt.quit(); }
  ...
  function checkHostSave() {
    ...
    check(Plugin.ScratchState.preferences.values.autoUpdates === false, "host choice is durably saved");
    check(!Plugin.ScratchState.updates.enabled, "update worker sees disabled updates");
    check(!Plugin.ScratchState.preferences.failed, "host save completed without an error");
  ...
  function checkStartupValues(language) {
    [one, two].forEach(function(widget) {
      suite.check(widget.workspaceName === "music", "startup keeps the configured scratchpad");
      ...
      suite.check(widget.preference("compact", false) && widget.uiScale === 1.25, "startup keeps layout choices");
      suite.check(widget.savedLabels.customLabels.active === "Music", "startup keeps nested preferences");
    });
  }
  ...
  QtObject {
    ...
    property var saved: suite.scenario === "removed-entry" || suite.scenario === "startup-restored"
      ? ({id:"sarr.scratchpeek"}) : (suite.startupCase ? suite.clone(suite.startupEntry)
        : ({id:"sarr.scratchpeek", autoUpdates:true, language:"en", _scratchpeekRevision:1}))
  ...
  Connections {
    ...
    function onAutoUpdatesChanged() {
      if (one.settingsReady && Plugin.ScratchState.preferences.hasSavedValues
          && one.autoUpdates !== Plugin.ScratchState.preferences.values.autoUpdates)
        suite.showedUnsavedChoice = true;
    }
  }
  Process {
    id: repair
    command: ["chmod", "700", Plugin.ScratchState.preferences.directory]
    onExited: function(code) {
      try {
        ...
        if (suite.startupCase) {
          ...
          var preferences = Plugin.ScratchState.preferences;
          suite.check(preferences.hasSavedValues && !preferences.failed, "retry establishes a saved configuration");
          suite.check(preferences.values.workspace === "music" && preferences.values.language === suite.expectedLanguage,
            "retry saves the full original host entry with the requested change");
          suite.check(!Plugin.ScratchState.updates.enabled, "retry preserves disabled updates");
        ...
        one.toggleUpdates();
        suite.check(!one.updatesSaveFailed && !one.autoUpdates && !two.autoUpdates, "retry commits on both monitors");
        suite.check(Plugin.ScratchState.preferences.values.autoUpdates === false, "worker sees the successful retry");
        suite.check(host.saved.autoUpdates === false, "host mirror follows the retry");
        suite.finish();
      } catch (error) { console.error("SCRATCHPEEK_PREFERENCES_FAIL: " + error); Qt.quit(); }
    }
  }
  Timer {
    ...
    onTriggered: {
      ...
      try {
        ...
        if (suite.scenario === "startup-restored") {
          ...
          suite.check(host.saved.workspace === "music" && host.saved.autoUpdates === false,
        ...
        if (suite.startupCase) {
          ...
          suite.check(!Plugin.ScratchState.updates.enabled, "unavailable preferences cannot start an update");
          suite.check(!one.setLanguage("fr"), "panel edit cannot bypass the failed save");
          suite.hostChange({workspace:"other", language:"de", autoUpdates:true});
          suite.checkStartupValues("pl");
          suite.check(JSON.stringify(host.saved) === JSON.stringify(suite.startupEntry), "failed host edit restores every original field");
          suite.check(!preferences.hasSavedValues && !Plugin.ScratchState.updates.enabled,
        ...
        if (suite.scenario === "host-change" || suite.scenario === "readonly-host") {
          ...
          suite.checkHostSave();
          var revision = preferences.values._scratchpeekRevision;
          suite.check(revision > stale._scratchpeekRevision, "host edit advances the saved revision");
          ...
          host.deliver(stale);
          suite.checkHostSave();
          suite.check(preferences.values._scratchpeekRevision === revision, "host echoes do not create new revisions");
          ...
          suite.check(one.autoUpdates && two.autoUpdates && Plugin.ScratchState.updates.enabled, "host can re-enable updates");
        ...
        if (suite.scenario === "readonly-settings") {
          suite.check(one.updatesSaveFailed && preferences.failed, "disk write failure reported");
          ...
          suite.check(!one.updatesSaveFailed && !one.autoUpdates && !two.autoUpdates, "durable save succeeds independently of host mirror");

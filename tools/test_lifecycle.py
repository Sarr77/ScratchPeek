...
def inside(base):
    ...
    with log_path.open('w') as log:
        try:
            ...
            assert (profile/'.local/state/scratchpeek/preferences.json').is_file()
            stop(); boot()
            run('omarchy','plugin','add',str(base/'source'),'--enable','--yes')
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='scratchpeek-lifecycle-') as directory:
        base = Path(directory)
        source = base / 'source'
        if args.remote:

  ...
  function receive(token, code, output) {
    ...
    if (phase === "snapshot") {
      ...
      if (state === "error") fail();
        ...
        if (!focused || current.previous !== job.plan.previous) fail();
